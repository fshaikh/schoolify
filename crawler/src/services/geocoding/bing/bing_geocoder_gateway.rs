...
use crate::utils::http_client::{get,HttpRequest,encode_url};
...
pub async fn fetch_geocode_location(request: &BingGeocodeGatewayRequest) -> Option<String> {
    ...
    if response.status_code != 200 {
        // what do we return
        println!("bing_geocoder_gateway::fetch_geocode_location - Error in forward geocoding: {}, {}", request.address, response.status_code);
        return None;
    }
...
fn construct_forward_geocode_url(request: &BingGeocodeGatewayRequest) -> String {
    // key={}&addressLine={}
    let url = format!("{}key={}&addressLine={}", GEOCODE_URL, API_KEY, request.address);
