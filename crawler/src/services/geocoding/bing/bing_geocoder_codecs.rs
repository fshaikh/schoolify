...
pub fn parse(input: &String) -> Option<ForwardGeocodeResponse>{
    let deserialized_result: Result<ForwardGeocodeResponse,serde_json::Error> = serde_json::from_str(input);
    match deserialized_result {
        Ok(result) => {
            if result.resourceSets[0].resources.len() == 0 {
                println!("bing_geocoder_codec::parse - Error decoding Bing Geocoder response: {}",input);
                return None;
            }
            return Some(result);
        }
        Err(err) => {
            println!("bing_geocoder_codec::parse - Error decoding Bing Geocoder response: {}",err);
            return None;
        }
    };
}
...
mod tests {
    ...
    fn it_should_decode_correctly() {
        ...
        assert_eq!(decoded.resourceSets[0].resources[0].point.coordinates[0],52.536488);
        assert_eq!(decoded.resourceSets[0].resources[0].point.coordinates[1],13.425927);
    }
}
//#endregion Tests
